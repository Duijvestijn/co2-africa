...
export default function ContactSection() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setStatus("loading");
    try {
      const res = await fetch("https://co2-africa.pages.dev/api/contact", {
        method: "POST",
          ...
          <motion.div variants={FU(0.2)} initial="hidden" animate={inView ? "visible" : "hidden"}
            ...
            {status === "success" ? (
              <div className="text-center py-12">
                <div className="w-14 h-14 rounded-full bg-af-green/10 flex items-center justify-center mx-auto mb-4">
                  <svg className="w-7 h-7 text-af-green" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
                </div>
                <h3 className="font-serif text-xl font-bold text-af-ink mb-2">Message sent</h3>
                <p className="text-af-muted text-sm">{"We'll be in touch shortly."}</p>
                <button onClick={() => setStatus("idle")} className="mt-6 text-af-green text-sm font-semibold hover:underline">Send another</button>
              </div>
            ) : (
              <form className="space-y-5" onSubmit={handleSubmit}>
                ...
                <button type="submit" disabled={status === "loading"}
                  className="w-full py-3.5 rounded-xl bg-af-green hover:bg-af-green-light disabled:opacity-60 text-white font-bold text-sm transition-colors shadow-md shadow-af-green/20">
