  ...
  return (
    <section ref={containerRef} id="hero" className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
      ...
      <motion.div style={{ y: bgY }} className="absolute inset-0 scale-110">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-af-dark/75 via-af-dark/40 to-af-dark/95" />
        <div className="absolute inset-0 bg-gradient-to-r from-af-dark/60 via-transparent to-af-dark/30" />
      </motion.div>
      ...
      <motion.div style={{ y: textY, opacity }} className="relative z-10 text-center px-6 max-w-5xl mx-auto pt-24">
        ...
        <motion.div custom={0} variants={FADE_UP} initial="hidden" animate="visible" className="inline-flex items-center gap-2 mb-8">
          <span className="w-8 h-px bg-af-sienna/60" />
          <span className="text-xs tracking-[0.3em] uppercase text-af-sienna font-medium">{t.label}</span>
          <span className="w-8 h-px bg-af-sienna/60" />
        </motion.div>
        ...
        <motion.p custom={2} variants={FADE_UP} initial="hidden" animate="visible"
          className="font-serif text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-[1.05] italic text-af-sienna mb-8">
          {t.titleLine2}
        </motion.p>

        {/* Description */}
        <motion.p custom={3} variants={FADE_UP} initial="hidden" animate="visible"
          className="text-af-sand/70 text-lg md:text-xl leading-relaxed max-w-3xl mx-auto mb-10">
          {t.description}
        </motion.p>
        ...
        <motion.div custom={4} variants={FADE_UP} initial="hidden" animate="visible"
          ...
          <a href="#what-is"
            className="px-7 py-3.5 rounded-xl text-sm font-semibold bg-af-sienna hover:bg-af-sienna-light text-white transition-colors shadow-lg shadow-af-sienna/20">
            {t.btn1}
          </a>
          <a href="#projects"
            className="px-7 py-3.5 rounded-xl text-sm font-medium bg-white/[0.07] hover:bg-white/[0.12] border border-white/[0.12] text-af-sand/80 hover:text-af-sand backdrop-blur-sm transition-colors">
            {t.btn2}
          </a>
        </motion.div>
        ...
        <motion.div custom={5} variants={FADE_UP} initial="hidden" animate="visible"
          ...
          {stats.map((stat, i) => (
            <div key={i} className="bg-af-dark/60 px-6 py-5 text-center">
              <div className="font-serif text-2xl md:text-3xl font-bold text-af-sienna mb-1">{stat.value}</div>
              <div className="text-af-sand/50 text-xs leading-snug">{stat.label}</div>
            </div>
          ))}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2 }} className="mt-12 flex flex-col items-center gap-2">
          <span className="text-af-sand/30 text-xs tracking-widest uppercase">
            {lang === "fr" ? "Défiler" : "Scroll"}
          </span>
          <motion.div animate={{ y: [0, 6, 0] }} transition={{ repeat: Infinity, duration: 2 }}
            className="w-px h-8 bg-gradient-to-b from-af-sienna/40 to-transparent" />
        </motion.div>
      </motion.div>
    </section>
  );
